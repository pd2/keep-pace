...
const maxLength = 52; // (Typically, the lower this number, the harder the puzzle.)
...
//    file: '',
  ...
  methods: {
    ...
    pickAQuote() {
      ...
      this.text = this.quotes[index].text;
      this.benefit = this.quotes[index].text.toUpperCase();
      this.lead = this.quotes[index].lead;
      this.category = this.quotes[index].category;
      this.points = this.quotes[index].points;
    ...
    guess(letter) {
      ...
      if (this.strikeout || this.puzzleComplete) {
        ...
        if (this.puzzleComplete) {
          this.score = this.score + Number(this.points)/100;
          if (this.score > 110) {
            this.score = Number(this.points)/100;
          }
          fireEmAll();
          this.share = true;
...
function ShareIt() {
  ...
  navigator.clipboard.writeText(copyText);
   ...
   if (navigator.canShare) {
    navigator.share({
      title: 'Share results',
      text: `#Keep-PACE I learnt how ${app.text} can reduce my carbon footprint in ${Math.round(elapsedTime/1000)} sec at ${linkURL}`,
     // url: linkURL,
    })
